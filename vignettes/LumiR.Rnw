...
XP.path <- system.file("extdata", package="LumiR")
@
<<read.experiment-2, eval=TRUE, message=FALSE>>=
bl <- read.experiment(XP.path)
@
<<read.experiment-3, echo=FALSE, warning=FALSE>>=
...
Since \texttt{slum} objects retain the phenotype information from the mapping 
file, they may be used as first argument in \texttt{plot\_layout}.


Additionally, \texttt{slummarize} does the standard curve fitting using a 
5-Parameters Logigistic curve by default. \texttt{LumiR} provides functions and 
methods to display the curves.

First, \texttt{plot\_SC} is a simple function that will plot the curves for each
plate and each analyte of a \texttt{slum} object. It does not offer much control
but is very easy to use.
<<plot_SC, fig.height=6.5>>=
plot_SC(sl)
@

The function is a wrapper around the method \texttt{geom\_sc}. Use the method 
directly for more customization possibilities. 
...
Printing \texttt{p} will result in the same plot as above. Any additional argument 
to \texttt{geom\_sc} will be passed to the underlying \texttt{ggplot2}'s method 
\texttt{geom\_line}.


\section{output}
A slum object can be written to a file following \htmladdnormallink{ImmPort}{https://immport.niaid.nih.gov/immportWeb/home/home.do?loginType=full} MBAA (Multiplex Bead Array Assays) format.
