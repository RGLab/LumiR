...
First, the name of the organisation producing the data should be added using the \texttt{set\_center} function.

<<set\_center>>=
sl <- set_center(sl, "FHCRC")
@

Then, \texttt{writeMBAA} can be used to write a csv file that can be opened using Excel and contain the fields required by ImmPort.
